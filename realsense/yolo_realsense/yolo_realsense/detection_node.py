...
class Yolov11Node(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("enable_bg_removal", True) 
        self.enable_bg_removal = self.get_parameter("enable_bg_removal").get_parameter_value().bool_value
    ...
    def object_segmentation(self):
        if self.enable_yolo and self.color_image_msg is not None and self.depth_image_msg is not None:
            ...
            if self.enable_bg_removal == False:
                ...
                np_depth_image = self.cv_bridge.imgmsg_to_cv2(self.depth_image_msg, 'passthrough')
            else:
                # Remove background
                bg_removed, np_color_image, np_depth_image = self.bg_removal(self.color_image_msg, self.depth_image_msg)
                self.get_logger().info("Succesfully removed background")
                yolo_input = bg_removed
            ...
            for result in results:
                ...
                self.get_logger().info(
                    f"YOLO Time: Total {total_time:.2f}ms "
                    f"(Pre: {preprocess_time:.2f}ms, Infer: {inference_time:.2f}ms, Post: {postprocess_time:.2f}ms)"
                )

            self.get_logger().info("Succesfully predicted")
