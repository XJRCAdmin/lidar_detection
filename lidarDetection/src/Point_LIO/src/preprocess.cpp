        ...
        if ((msg->points[i].line < N_SCANS) &&
            ...
            if (valid_num % point_filter_num == 0) {
                ...
                pl_full[i].curvature = msg->points[i].offset_time /
                                       float(1000000); // curvature unit: ms
...
void Preprocess::livox_pcl2_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg) {
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
    ...
    for (uint i = 1; i < plsize; i++) {
        ...
        if ((src_pt.line < N_SCANS) &&
            ((src_pt.tag & 0x30) == 0x10 || (src_pt.tag & 0x30) == 0x00)) {
            valid_num++;
            if (valid_num % point_filter_num == 0) {
                pl_full[i].x = src_pt.x;
                pl_full[i].y = src_pt.y;
                pl_full[i].z = src_pt.z;
                pl_full[i].intensity = src_pt.intensity;
                pl_full[i].curvature = src_pt.timestamp / float(1000000); // curvature unit: ms
