...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  if (cut_frame) {
    ...
    for (int i = 0; i < ptr->size(); i++) {
      ...
      if (
        ptr->points[i].curvature / double(1000) + get_time_sec(msg->header.stamp) - time_div >
...
void imu_cbk(const sensor_msgs::msg::Imu::SharedPtr msg_in)
{
  ...
  static double imu_time_offset = 0.0;
  const double raw_time = get_time_sec(msg_in->header.stamp) - time_lag_imu_to_lidar;
  double timestamp = raw_time + imu_time_offset;

  mtx_buffer.lock();

  if (timestamp < last_timestamp_imu) {
    double jump = last_timestamp_imu - timestamp + 1e-3;
    imu_time_offset += jump;
    timestamp = raw_time + imu_time_offset;
    RCLCPP_WARN(
      logger, "imu time went backwards (raw %.6f < last %.6f), apply offset +%.6f and reset buffer", raw_time,
      last_timestamp_imu, jump);
    imu_deque.clear();
  }
...
void map_incremental()
{
  ...
  for (int i = 0; i < feats_down_size; i++) {
    if (!Nearest_Points[i].empty()) {
      ...
      if (
        fabs(points_near[0].x - mid_point.x) > 1.732 * filter_size_map_min ||
      ...
      for (int readd_i = 0; readd_i < points_near.size(); readd_i++) {
        ...
        if (
          fabs(points_near[readd_i].x - mid_point.x) < 0.5 * filter_size_map_min &&
  ...
  std::shared_ptr<tf2_ros::TransformBroadcaster> & tf_br)
{
  ...
  // {
  //     const auto &pose_in = odomAftMapped.pose.pose;
  //     Eigen::Quaterniond q(pose_in.orientation.w, pose_in.orientation.x, pose_in.orientation.y, pose_in.orientation.z);
  //     q.normalize();
  //     Eigen::Quaterniond q_inv = q.conjugate();
  //     Eigen::Vector3d p(pose_in.position.x, pose_in.position.y, pose_in.position.z);
  //     Eigen::Vector3d p_inv = -(q_inv * p);

  //     odomAftMapped.pose.pose.position.x = p_inv.x();
  //     odomAftMapped.pose.pose.position.y = p_inv.y();
  //     odomAftMapped.pose.pose.position.z = p_inv.z();
  //     odomAftMapped.pose.pose.orientation.x = q_inv.x();
  //     odomAftMapped.pose.pose.orientation.y = q_inv.y();
  //     odomAftMapped.pose.pose.orientation.z = q_inv.z();
  //     odomAftMapped.pose.pose.orientation.w = q_inv.w();

  //     Eigen::Vector3d v(odomAftMapped.twist.twist.linear.x,
  //                       odomAftMapped.twist.twist.linear.y,
  //                       odomAftMapped.twist.twist.linear.z);
  //     Eigen::Vector3d w(odomAftMapped.twist.twist.angular.x,
  //                       odomAftMapped.twist.twist.angular.y,
  //                       odomAftMapped.twist.twist.angular.z);
  //     Eigen::Vector3d v_inv = q_inv * v;
  //     Eigen::Vector3d w_inv = q_inv * w;
  //     odomAftMapped.twist.twist.linear.x = v_inv.x();
  //     odomAftMapped.twist.twist.linear.y = v_inv.y();
  //     odomAftMapped.twist.twist.linear.z = v_inv.z();
  //     odomAftMapped.twist.twist.angular.x = w_inv.x();
  //     odomAftMapped.twist.twist.angular.y = w_inv.y();
  //     odomAftMapped.twist.twist.angular.z = w_inv.z();
  // }
...
void publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr & pubPath)
{
  ...
  // {
  //     const auto &pose_in = msg_body_pose.pose;
  //     Eigen::Quaterniond q(pose_in.orientation.w, pose_in.orientation.x, pose_in.orientation.y, pose_in.orientation.z);
  //     q.normalize();
  //     Eigen::Quaterniond q_inv = q.conjugate();
  //     Eigen::Vector3d p(pose_in.position.x, pose_in.position.y, pose_in.position.z);
  ...
  //     msg_body_pose.pose.position.x = p_inv.x();
  //     msg_body_pose.pose.position.y = p_inv.y();
  //     msg_body_pose.pose.position.z = p_inv.z();
  //     msg_body_pose.pose.orientation.x = q_inv.x();
  //     msg_body_pose.pose.orientation.y = q_inv.y();
  //     msg_body_pose.pose.orientation.z = q_inv.z();
  //     msg_body_pose.pose.orientation.w = q_inv.w();
  // }
...
int main(int argc, char ** argv)
{
  ...
  if (p_pre->lidar_type == AVIA) {
    if (use_livox_custom_msg) {
      sub_pcl_livox =
        nh->create_subscription<livox_ros_driver2::msg::CustomMsg>(lid_topic, rclcpp::SensorDataQoS(), livox_pcl_cbk);
    } else {
      sub_pcl =
        nh->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, rclcpp::SensorDataQoS(), standard_pcl_cbk);
    }
  } else {
    sub_pcl =
      nh->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, rclcpp::SensorDataQoS(), standard_pcl_cbk);
  ...
  if (odom_only) {
    ...
    pubOdomAftMapped = nh->create_publisher<nav_msgs::msg::Odometry>("/Odometry", 100000);
  }
    ...
    if (sync_packages(Measures)) {
          ...
          if (publish_odometry_without_downsample) {
            ...
            if (runtime_pos_log) {
              ...
              fout_out << setw(20) << Measures.lidar_beg_time - first_lidar_time << " " << euler_cur.transpose() << " "
                       << state_out.pos.transpose() << " " << state_out.vel.transpose() << " "
                       << state_out.omg.transpose() << " " << state_out.acc.transpose() << " "
        ...
        for (k = 0; k < time_seq.size(); k++) {
          ...
          if (publish_odometry_without_downsample) {
            ...
            if (runtime_pos_log) {
              ...
              fout_out << setw(20) << Measures.lidar_beg_time - first_lidar_time << " " << euler_cur.transpose() << " "
                       << state_in.pos.transpose() << " " << state_in.vel.transpose() << " " << state_in.bg.transpose()
      ...
      if (runtime_pos_log) {
        ...
        printf(
          ...
          "map incre: %0.6f ave total: %0.6f icp: %0.6f propogate: %0.6f \n",
